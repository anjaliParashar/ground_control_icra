...
void AckermannCmdMuxNodelet::ackermannCmdCallback(const f1tenth_msgs::F1TenthDriveStamped::ConstPtr& msg, unsigned int idx)
{
...
#endif
	mux_ackermann_cmd_pub = nh_priv.advertise <f1tenth_msgs::F1TenthDriveStamped> (output_name, 10);
	...
	**********************/
	...
	for (unsigned int i = 0; i < ackermann_cmd_sub.size(); i++)
	{
		ackermann_cmd_sub[i].subs =
				nh_priv.subscribe<f1tenth_msgs::F1TenthDriveStamped>(ackermann_cmd_sub[i].topic, 10, AckermannCmdFunctor(i, this));
