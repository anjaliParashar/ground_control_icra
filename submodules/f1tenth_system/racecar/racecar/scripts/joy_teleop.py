...
class JoyTeleop:
    ...
    def __init__(self):
        ...
        rospy.Timer(rospy.Duration(1 / 50.0), self.check_command)
    ...
    def match_command(self, c, buttons):
        ...
        any_commands_matched = np.any(
            [np.array_equal(command["buttons"], button_indexes) for name, command in self.command_list.items()]
        )
        ...
        if cmd["type"] == "topic":
            ...
            if cmd["action_name"] in self.offline_actions:
                rospy.logerr(
                    ...
                    "server was unavailable. Trying to reconnect...".format(cmd["action_name"])
                )
            ...
            if cmd["service_name"] in self.offline_services:
                rospy.logerr(
                    ...
                    "server was unavailable. Trying to reconnect...".format(cmd["service_name"])
                )
            ...
            raise JoyTeleopException(
                "command {} is neither a topic publisher nor an action or service client".format(command)
            )

    def run_topic(self, c, joy_state):
        cmd = self.command_list[c]
        msg_type = self.get_message_type(cmd["message_type"])
        msg = msg_type()
        ...
        for mapping in cmd.get("axis_mappings", []):
    ...
    def run_service(self, c, joy_state):
        ...
        if not self.srv_clients[cmd["service_name"]](request):
            rospy.loginfo(
                "Not sending new service request for command {} because previous request has not finished".format(c)
            )
